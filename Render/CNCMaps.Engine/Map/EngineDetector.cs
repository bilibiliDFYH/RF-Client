...
namespace CNCMaps.Engine.Map {
	public class EngineDetector {
		...
		private static double PercentageObjectsKnown(MapFile mf, VirtualFileSystem vfs, IniFile rules, TheaterSettings ths) {
            ...
            var tiles = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Tiles.Where(t => t != null), t => t.TileNum);
            ...
            var infs = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Infantries, o => o.Name);
			known += infs.Count(o => objectKnown(o, rules.GetSection("InfantryTypes")));
			total += infs.Count();

            var terrains = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Infantries, o => o.Name);
			known += terrains.Count(o => objectKnown(o, rules.GetSection("TerrainTypes")));
			total += terrains.Count();

            var units = mf.Infantries.GroupBy(o => o.Name).Select(g => g.First());
			known += units.Count(o => objectKnown(o, rules.GetSection("VehicleTypes")));
			total += units.Count();

            var aircrafts = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Aircrafts, o => o.Name);
			known += aircrafts.Count(o => objectKnown(o, rules.GetSection("AircraftTypes")));
			total += aircrafts.Count();

            var smudges = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Smudges, o => o.Name);
			known += smudges.Count(o => objectKnown(o, rules.GetSection("SmudgeTypes")));
			total += smudges.Count();

            var structures = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Structures, o => o.Name);
            ...
            var overlays = CNCMaps.Shared.Utility.Compare.DistinctBy(mf.Overlays, o => o.Number);
